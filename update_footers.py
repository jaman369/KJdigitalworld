...
def process_file(filepath):
    try:
        ...
        content_no_wa = re.sub(r'<a[^>]*class="wa-float"[^>]*>.*?</a>', '', new_content, flags=re.DOTALL)
        content_no_wa = re.sub(r'<style>\s*\.wa-float\s*\{.*?</style>', '', content_no_wa, flags=re.DOTALL)
        
        # We need to re-append the footer_html styling because we stripped it along with wa-float if we did it out of order.
        # Actually my original script replaced the footer FIRST, then removed all wa-floats, which also removed the one from footer_html!
        # And then replaced footer AGAIN!
        # Let's fix that logic:
    except Exception as e:
        print(f"Error reading {filepath}: {e}")
        return

    # Remove all existing wa-float and styles
    content_clean = re.sub(r'<a[^>]*class="wa-float"[^>]*>.*?</a>', '', content, flags=re.DOTALL)
    content_clean = re.sub(r'<style>\s*\.wa-float\s*\{.*?</style>', '', content_clean, flags=re.DOTALL)
    
    # Now replace the footer with our new footer_html (which includes the wa-float)
    final_content = re.sub(r'<footer\b[^>]*>.*?</footer>', footer_html, content_clean, flags=re.DOTALL)
